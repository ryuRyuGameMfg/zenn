...
- **OKR（2026-04-01改訂・非公式API運用）**:
  - O1 フォロワー 1000人: 37/1000（3.7%）
  - O2 累計スキ数 1000: 189/1000（18.9%）
  - O3 平均スキ数 15以上維持: 5.6/15（目標未達）
- **統計取得方式**: Zenn非公式API（Playwright方式は保留）
...
- **統計取得は非公式APIで十分**（2026-04-01）：Playwright実装は完了したが、非公式APIで十分な精度が得られるため保留。OKRをスキ数ベースに統一し、運用コスト削減。
